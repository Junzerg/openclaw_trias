...
export class OpenClawAdapter {
  ...
  async callLLM(systemPrompt: string, userMessage: string, model?: string): Promise<LLMResponse> {
    ...
    const args = ['agent', '--agent', this.config.agentId, '--session-id', sessionId, '--message', fullMessage];
  ...
  async executeCode(
    ...
    const sessionId = randomUUID();
    const args = ['agent', '--agent', this.config.agentId, '--session-id', sessionId, '--message', prompt];
