...
export interface Voter {
  role: string;
  vote(proposal: string, taskId?: string): Promise<VoteOutcome>;
...
export class DebateEngine {
  ...
  async runDebate(
    ...
    let currentProposal = await radical.propose(petition, taskId);
    ...
    for (let roundNum = 1; roundNum <= this.config.max_rounds; roundNum++) {
      ...
      // 首轮为批评，后续为反驳批评
      if (roundNum === 1) {
        critiqueText = await conservative.critique(currentProposal, taskId);
      } else {
        critiqueText = await conservative.rebut(currentProposal, taskId);
      }
      ...
      if (roundNum < this.config.max_rounds) {
        lastRebuttal = await radical.rebut(critiqueText, taskId);
        radical.emitEvent(EventAction.PROPOSE, { 
...
export class VotingMachine {
  async tally(proposal: string, voters: Voter[], voteRound: number = 99, taskId?: string): Promise<VoteResult> {
    ...
    for (const voter of voters) {
      const outcome = await voter.vote(proposal, taskId);
