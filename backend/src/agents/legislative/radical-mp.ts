...
export class RadicalMP extends BaseAgent {
  ...
  async propose(petition: string, _taskId?: string): Promise<string> {
    this.requirePermission(Permission.PLAN);
  ...
  async rebut(critique: string, _taskId?: string): Promise<string> {
    this.requirePermission(Permission.PLAN);
  ...
  async vote(proposal: string, _taskId?: string): Promise<VoteOutcome> {
    this.requirePermission(Permission.PLAN);
    const prompt = `作为激进派议员，请对以下提案投票（赞成/反对）。无论之前是否已达成共识，你必须且只能明确回复包含“赞成”或“反对”字样：\n\n<proposal>\n${proposal}\n</proposal>`;
    const result = await this.callLLM(prompt, _taskId);
