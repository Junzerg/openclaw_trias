...
class CyberGovernment:
        ...
        *,
        ...
        for attempt in range(1, max_retries + 1):
            logger.info(
                ...
                attempt, max_retries, bill_id,
            )

            result = await self._run_pipeline(
                petition, lifecycle, bill_id,
            )
    ...
    async def _run_pipeline(
        ...
        act = await self.speaker.generate_act(
            petition, debate_result, vote_result,
        )
        ...
        report = await self.execution_engine.execute_act(
            act, event_publisher=execution_publisher
        )
    ...
    async def _publish_propose(self, bill_id: str, agent: str, text: str, emotion: str = "neutral", round_number: int = 1, conflict_score: float = 0.0) -> None:
        """发布提案/反驳事件。"""
        event = DebateEvent(
            source_agent=agent,
    ...
    async def _publish_vote_passed(self, bill_id: str, ayes: int, nays: int, act: Any | None = None) -> None:
        """发布表决通过事件。"""
        payload = {}
        if act is not None:
    ...
    async def _publish_tool_call(self, bill_id: str, skill: str, step_index: int, status: str) -> None:
        """发布工具调用事件。"""
    ...
    def _force_vote_passed(vote_result: VoteResult) -> VoteResult:
        ...
        return VoteResult(
            ...
            records=[
                VoteRecord(voter_role=r.voter_role, vote=True)
                for r in vote_result.records
            ],
