...
class RulesEngine:
    ...
    def __init__(
        ...
        *,
        ...
        self._max_execution_time: int = constitution.security.max_execution_time_seconds
        self._deviation_max: float = constitution.judicial.deviation.max_score
        self._deviation_scorer: DeviationScorer = deviation_scorer or _default_deviation_scorer
    ...
    def check_resource_usage(
        ...
        if tokens_consumed > self._max_tokens:
            return RuleCheckResult(
                ...
                violation_detail=(f"Token 消耗 {tokens_consumed} 超过上限 {self._max_tokens}"),
            )
        if execution_time > self._max_execution_time:
            return RuleCheckResult(
                ...
                rule_name="resource_usage",
                violation_detail=(
                    f"执行时间 {execution_time:.1f}s 超过上限 {self._max_execution_time}s"
                ),
            )
    ...
    async def check_deviation(
        ...
        explanation = f"偏离度 {score:.2f} {'<=' if passed else '>'} 阈值 {self._deviation_max:.2f}"
