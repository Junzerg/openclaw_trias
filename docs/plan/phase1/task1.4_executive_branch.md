...
- [ ] 总统可根据 Token 预算和 Skill 可用性决定签署/否决
- [ ] Veto 机制正确触发并生成包含理由的否决通知
- [ ] 工程部长和国务卿各自只能使用挂载的 Skill
- [ ] 执行引擎可按法案步骤顺序执行（Mock 工具调用）
- [ ] 执行引擎正确处理步骤依赖关系
- [ ] 执行报告包含各步骤结果和 Token 消耗统计
- [ ] RBAC 校验：行政 Agent 无法执行 PLAN 操作（总统除外）
- [ ] 所有单测通过
