...
function appReducer(state: AppState, action: AppAction): AppState {
  switch (action.type) {
    ...
    case 'DEBATE_EVENT': {
      ...
      const isVoting = statement?.includes('[VOTING]');

      if (conflict_score !== undefined && !isVoting) {
      ...
      if (conflict_score !== undefined && !isVoting && (newScores.length === 0 || newScores[newScores.length - 1] !== conflict_score)) {
