...
/**
 * Buffer to hold the currently active stream content.
 * Solves the race condition where backend starts streaming before the frontend Phaser Scene
 * has finished its transition fade-in (e.g. Parliament -> Executive).
 */
export const activeStreamState: { agent: string; content: string; completed: boolean; lastUpdate: number } = {
  agent: '',
  content: '',
  completed: true,
  lastUpdate: 0,
...
export function useWebSocket(taskId?: string) {
  ...
  const processPayload = useCallback((payload: any) => {
    ...
    if (payload.action === 'stream_chunk') {
      ...
      const agent = p.agent || payload.source_agent || 'unknown';
      const chunk = p.chunk || '';
      const completed = !!p.completed;

      // Update the global active stream buffer
      const now = Date.now();
      if (activeStreamState.agent !== agent || (activeStreamState.completed && now - activeStreamState.lastUpdate > 2000)) {
        // New stream started
        activeStreamState.agent = agent;
        activeStreamState.content = chunk;
        activeStreamState.completed = completed;
      } else {
        // Continuing stream
        activeStreamState.content += chunk;
        activeStreamState.completed = completed;
      }
      activeStreamState.lastUpdate = now;

      streamChunkBus.next({ chunk, agent, completed });
      
      // Let it fall through so AppReducer (and DebateLogPanel) gets the STREAM_CHUNK event!
    }
