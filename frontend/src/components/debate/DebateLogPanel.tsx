...
export function DebateLogPanel() {
  ...
  // Auto-scroll to bottom when new rounds/content arrive (ResizeObserver captures typewriter/stream growth)
  useEffect(() => {
    if (!containerRef.current) return;
    
    // Find the scrollable parent (.panel-content) to check if user scrolled up
    const scrollParent = containerRef.current.closest('.panel-content');
    
    const observer = new ResizeObserver(() => {
      let isUserScrolledUp = false;
      
      if (scrollParent) {
        // If user scrolled up more than ~50px from bottom, don't force scroll down
  ...
  return (
    <div className="debate-log-panel" ref={containerRef}>
